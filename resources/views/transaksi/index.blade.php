...
<script>
  $(function() {
    ...
    $(".menu-item .card li").click(function() {
        ...
        const existingItem = orderedList.find(item => item.menu_id === id);
        if (existingItem) {
            existingItem.qty++;
            $(`.ordered-list-item[data-id="${id}"] .qty-item`).val(existingItem.qty);
            $(`.ordered-list-item[data-id="${id}"] .subtotal`).text(`Sub Total : Rp. ${harga * existingItem.qty}`);
        } else {
            const newItem = {
                'menu_id': id,
                'menu': menu_clicked,
            ...
            let listOrder = `
                <li class="ordered-list-item" data-id="${id}">
                    <h6>${menu_clicked}</h6>
